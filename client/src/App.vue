  ...
  <loading-overlay :active="isLoading" :is-full-page="true" :loader="'spinner'" />
    ...
    >
      ...
      <ul v-if="user" class="list-unstyled components">
        <li>
          <!-- :class="{ 'has-text-white': darkMode, 'sidebar-active': $route.name == 'ReportView' }" -->
          <a
            class="sidebar-link"
            :class="activeSidebarClass('ReportView')"
            disabled
            @click="$router.push('/')"
            >หน้าหลัก</a
          >
        ...
        <li v-if="user.USER_TYPE == 'member'">
          <a
            class="sidebar-link"
            :class="activeSidebarClass('SentReport')"
            @click="$router.push('/sent-report')"
            >แจ้งปัญหา</a
          >
        </li>
        <li>
            ...
            <div class="dropdown-menu" id="dropdown-alert" role="menu">
              <div class="dropdown-content mr-4">
                ...
                <a v-for="(alert, index) in arrAlert" 
                  ...
                  @click="$router.push(`/report/${alert.REPORT_ID}`)"
                >
                <span>{{ (index+1)+".ระบบ: "+alert.SYSTEM_NAME }}</span>
